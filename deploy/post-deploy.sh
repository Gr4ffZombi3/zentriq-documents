...
curl_final_headers() {
    curl -sS -L --connect-timeout 10 -D - -o /dev/null "$@" || true
}
...
curl_effective_field() {
    local field_name="${1:-}"
    shift || true
    curl -sS -L --connect-timeout 10 -o /dev/null -w "%{${field_name}}" "$@" || true
}

curl_content_type() {
    curl_effective_field "content_type" "$@"
}

curl_effective_url() {
    curl_effective_field "url_effective" "$@"
}
...
main() {
    local PUBLIC_HOST_VALUE PUBLIC_BASE_URL CSS_PATH FAILED
    ...
    FLASK_CSS_TYPE="$(curl_content_type http://127.0.0.1:8000/static/css/app.css)"
    ...
    if [ -z "$PUBLIC_HOST_VALUE" ]; then
        ...
        local -a LOCAL_RESOLVE_ARGS
        local LOCAL_LOGIN_URL LOCAL_LOGIN_STATUS LOCAL_LOGIN_HTML LOCAL_CSS_URL LOCAL_CSS_STATUS
        local LOCAL_CSS_HEADERS LOCAL_CSS_TYPE LOCAL_CSS_HEADER_TYPE LOCAL_CSS_BYTES LOCAL_CSS_EFFECTIVE_URL
        ...
        LOCAL_CSS_HEADERS="$(curl_final_headers "${LOCAL_RESOLVE_ARGS[@]}" "$LOCAL_CSS_URL")"
        LOCAL_CSS_HEADER_TYPE="$(printf '%s' "$LOCAL_CSS_HEADERS" | extract_last_header_value "Content-Type")"
        LOCAL_CSS_TYPE="$(curl_content_type "${LOCAL_RESOLVE_ARGS[@]}" "$LOCAL_CSS_URL")"
        if [ -z "$LOCAL_CSS_TYPE" ]; then
            LOCAL_CSS_TYPE="$LOCAL_CSS_HEADER_TYPE"
        fi
        LOCAL_CSS_EFFECTIVE_URL="$(curl_effective_url "${LOCAL_RESOLVE_ARGS[@]}" "$LOCAL_CSS_URL")"
        ...
        if [ "$LOCAL_CSS_STATUS" = "200" ] && printf '%s' "$LOCAL_CSS_TYPE" | grep -qi '^text/css'; then
            echo "    Nginx CSS OK (${LOCAL_CSS_BYTES} Bytes, ${LOCAL_CSS_TYPE}, final ${LOCAL_CSS_EFFECTIVE_URL})"
        else
            echo "    FEHLER: Nginx-CSS fehlerhaft (HTTP ${LOCAL_CSS_STATUS}, Content-Type '${LOCAL_CSS_TYPE:-${LOCAL_CSS_HEADER_TYPE:-unbekannt}}', final ${LOCAL_CSS_EFFECTIVE_URL:-unbekannt})."
            FAILED=1
        fi
    ...
    if [ -z "$PUBLIC_BASE_URL" ]; then
        ...
        local PUBLIC_LOGIN_URL PUBLIC_LOGIN_STATUS PUBLIC_CSS_URL PUBLIC_CSS_STATUS PUBLIC_CSS_TYPE
        local PUBLIC_CSS_BYTES PUBLIC_LOGIN_HTML
        ...
        PUBLIC_CSS_TYPE="$(curl_content_type "$PUBLIC_CSS_URL")"
